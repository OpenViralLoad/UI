...
# This is the counter used to generate the ID that the user can use to
# access the port.
deviceIdCounter = 0
...
# Returns JSON array with the serial_number, device_subscription_id, and
# device_port
    ...
    for connectable_port in serial.tools.list_ports.comports():
        ...
        for deviceSerialNum in arrayOfSerialNums:
            if deviceSerialNum == serialNumFromPort:
                ...
                id_with_port_dictionary[deviceId] = port
                lock.release()
                connected_devices['devices_connected'].append(
                    {'serial_number': deviceSerialNum, 'device_subscription_id': deviceId, 'device_port': connectable_port})
